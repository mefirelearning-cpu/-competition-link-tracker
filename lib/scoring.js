...
export async function adminAdjustPoints({
  ...
  return withTransaction(async client => {
    ...
    const safeOperationId=String(operationId||"").trim().slice(0,120) || transactionId;
    const idempotencyKey="admin_adjustment:"+competitionId+":"+participantId+":"+safeOperationId;
    ...
    const totalPoints=Math.max(0,Number(ledgerTotal.rows[0]?.total||0));
    ...
    const existing=inserted.rowCount
      ? inserted.rows[0]
      : (await client.query(
          "SELECT id,final_points FROM point_transactions WHERE idempotency_key=$1 LIMIT 1",
          [idempotencyKey]
        )).rows[0];
    ...
    return {
      ...
      participantId,
      referralCode:member.rows[0].referral_code,
      appliedAmount:inserted.rowCount ? applied : 0,
      totalPoints,
