...
export async function awardValidClick({
  ...
  return withTransaction(async (client) => {
    ...
    let dailyCapPoints = rule.daily_cap_points;
    ...
    if (campaignId) {
      ...
      if (campaign && campaign.status === "active") {
        ...
        dailyCapPoints = campaign.click_daily_cap === null
          ? rule.daily_cap_points
          : campaign.click_daily_cap;
      }
    }
    ...
    if (dailyCapPoints !== null && dailyCapPoints !== undefined) {
      ...
      const remaining = Math.max(0, Number(dailyCapPoints) - todayTotal);
...
export async function awardLedgerEvent({
  ...
  return withTransaction(async (client) => {
    ...
    if (competition.status !== "active" && createdBy === "system") {
      const totalPoints = await refreshPointCache(client, competitionId, participantId);
      return { inserted: false, totalPoints, reason: "competition_not_active" };
    }
    ...
    return {
      ...
      awarded: inserted ? Number(inserted.final_points || 0) : 0,
      totalPoints,
