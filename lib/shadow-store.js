...
async function ensureCompetition(client, comp) {
  await client.query(
    `INSERT INTO competitions
      (id, slug, name, description, status, starts_at, ends_at, timezone,
       registrations_open, winner_count, max_participants, rules, settings,
     ...
     ON CONFLICT (id) DO UPDATE SET
       name = EXCLUDED.name,
       description = CASE WHEN EXCLUDED.description <> '' THEN EXCLUDED.description ELSE competitions.description END,
       status = EXCLUDED.status,
       starts_at = COALESCE(EXCLUDED.starts_at, competitions.starts_at),
       ends_at = COALESCE(EXCLUDED.ends_at, competitions.ends_at),
       timezone = COALESCE(NULLIF(EXCLUDED.timezone,''), competitions.timezone),
       registrations_open = EXCLUDED.registrations_open,
       winner_count = EXCLUDED.winner_count,
       max_participants = COALESCE(EXCLUDED.max_participants, competitions.max_participants),
       rules = CASE WHEN EXCLUDED.rules <> '' THEN EXCLUDED.rules ELSE competitions.rules END,
    ...
    [
      ...
      comp.startsAt || null,
      comp.endsAt || null,
      comp.timezone || "Africa/Douala",
      comp.registrationsOpen !== false,
      Math.max(1, Number(comp.winnerCount || 1)),
      comp.maxParticipants ? Math.max(1, Number(comp.maxParticipants)) : null,
      comp.rules || "",
      JSON.stringify({
        shadowSynced: true,
  ...
  await client.query(
    `INSERT INTO point_rules
     ...
     VALUES ($1,$2,'valid_click',TRUE,1,1,60,'{"seededDefault":true}'::jsonb)
     ON CONFLICT (competition_id,action_type) DO NOTHING`,
    [stableId("rule",comp.id+":valid_click"),comp.id]
  ).catch(()=>{});
