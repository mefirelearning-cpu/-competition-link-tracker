...
export async function getCampaignStats(competitionId) {
  const result = await query(
    `SELECT c.id, c.name, c.product, c.status, c.featured,
            ...
            COUNT(DISTINCT sa.id)::int AS sales,
            COALESCE(SUM(DISTINCT CASE WHEN sa.status = 'confirmed' THEN sa.amount ELSE 0 END),0) AS attributed_revenue
     FROM campaigns c
     LEFT JOIN campaign_share_events s ON s.campaign_id = c.id
     LEFT JOIN interests i ON i.campaign_id = c.id
     LEFT JOIN leads l ON l.campaign_id = c.id AND l.status = 'confirmed'
     LEFT JOIN sales sa ON sa.campaign_id = c.id AND sa.status = 'confirmed'
