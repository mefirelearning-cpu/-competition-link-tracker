...
export async function getParticipantRankEvolution(competitionId,participantId) {
  ...
  const previousSnapshot=history.rows[1]||null;
  const currentRank=Number(currentRow?.rank || latestSnapshot?.rank || 0) || null;
  const previousRank=Number(previousSnapshot?.rank || 0) || null;
  const rankDelta=currentRank && previousRank ? previousRank-currentRank : null;

  return {
    currentRank,
    currentPoints:Number(currentRow?.points || latestSnapshot?.points || 0),
    previousRank,
    previousSnapshotAt:previousSnapshot?.bucket_at || null,
