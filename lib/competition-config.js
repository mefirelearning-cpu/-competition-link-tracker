...
export async function resizeCompetitionDays({
  ...
  return withTransaction(async (client) => {
    const competitionResult=await client.query(
      `SELECT id,starts_at,ends_at,timezone,settings
       FROM competitions
       WHERE id=$1
       FOR UPDATE`,
      [competitionId]
      ...
      for (let n = current + 1; n <= target; n++) {
        ...
        if(competition.starts_at){
          const schedule=await client.query(
            ...
            [competition.starts_at,n]
          );
          startsAt=schedule.rows[0].starts_at;
          endsAt=schedule.rows[0].ends_at;
        }

        await client.query(
          `INSERT INTO competition_days
            (id, competition_id, day_number, title, status,
             reward_daily_points, starts_at, ends_at)
           VALUES ($1,$2,$3,$4,'draft',0,$5,$6)
           ON CONFLICT (competition_id, day_number) DO NOTHING`,
          [id("day"), competitionId, n, "Jour " + n, startsAt, endsAt]
        );
      }
    }

    let newEndsAt=competition.ends_at;
    if(competition.starts_at){
      const endResult=await client.query(
        `SELECT $1::timestamptz + ($2::int * interval '1 day') AS ends_at`,
        [competition.starts_at,target]
      );
      newEndsAt=endResult.rows[0].ends_at;
    }

    await client.query(
      `UPDATE competitions
       SET ends_at = COALESCE($2,ends_at),
      ...
      [
        competitionId,
        newEndsAt,
        JSON.stringify({ configuredDayCount: target })
      ]
    );
    ...
    await client.query(
      ...
      [
        ...
        competitionId,
        JSON.stringify({
          previousDayCount: current,
          newDayCount: target,
          previousEndsAt:competition.ends_at,
          newEndsAt,
          force
        })
      ]
...
export async function runLifecycleSweep() {
  ...
  for(const row of candidates.rows){
    try{
      const state=await ensureCompetitionLifecycle(row.id);
      let snapshot=null;
      if(state && (state.status==="active" || state.status==="completed")){
        snapshot=await snapshotCompetitionRanks(row.id);
      }
      results.push({
        id:row.id,
        status:state?.status || null,
        leaderboardFrozen:Boolean(state?.leaderboard_frozen),
        rankSnapshot:snapshot
      });
    }catch(error){
      results.push({
        id:row.id,
        error:String(error?.message || error || "lifecycle_failed").slice(0,300)
      });
    }
  }
