...
export async function trackReferralVisit({
  ...
  if (rapidCount >= Number(fraud?.suspicious_threshold || 20)) {
    createFraudFlag({
      competitionId,
      participantId: null,
      visitorId: visitorHash,
      riskScore: Math.min(1, rapidCount / Math.max(1, Number(fraud?.suspicious_threshold || 20))),
      reason: "Rechargements rapides répétés",
      metadata: { rapidCount, burstWindowSeconds }
    }).catch(() => {});
  }
