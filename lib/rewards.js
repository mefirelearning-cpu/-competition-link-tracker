...
export async function addRewardTier({
  ...
  const min=Math.max(0,Number.parseInt(String(minPoints),10)||0);
  const max=String(maxPoints??"").trim()===""?null:Math.max(0,Number.parseInt(String(maxPoints),10)||0);
  if(max!==null && max<min) return {ok:false,reason:"invalid_range"};
  ...
  return withTransaction(async client=>{
    const overlap=await client.query(
      `SELECT id,min_points,max_points
       ...
       WHERE competition_id=$1
         AND enabled=TRUE
         AND min_points <= COALESCE($3::int,2147483647)
         AND COALESCE(max_points,2147483647) >= $2
       LIMIT 1`,
      [competitionId,min,max]
    );
    if(overlap.rowCount){
      return {ok:false,reason:"overlap",conflict:overlap.rows[0]};
    }
    ...
    await client.query(
      ...
      [id("audit"),adminId,tierId,JSON.stringify({competitionId,minPoints:min,maxPoints:max})]
    );
...
export async function updatePrize({
  ...
  return withTransaction(async client=>{
    const before=await client.query(
      "SELECT * FROM prizes WHERE id=$1 AND competition_id=$2 FOR UPDATE",
      [prizeId,competitionId]
    );
    if(!before.rowCount) return {ok:false,reason:"not_found"};
    await client.query(
      `UPDATE prizes
       SET name=$3,description=$4,duration_text=$5,sort_order=$6,updated_at=NOW()
       WHERE id=$1 AND competition_id=$2`,
      [
        prizeId,competitionId,cleanName,
        String(description||"").trim().slice(0,1000),
        String(durationText||"").trim().slice(0,120),
        Number.parseInt(String(sortOrder),10)||0
      ]
    ...
    await client.query(
      `INSERT INTO admin_audit_logs
       (id,admin_id,action,entity_type,entity_id,description,metadata)
       VALUES ($1,$2,'prize_updated','prize',$3,'Lot modifié',$4::jsonb)`,
      [id("audit"),adminId,prizeId,JSON.stringify({competitionId,before:before.rows[0]})]
    );
    return {ok:true};
  });
}
...
export async function deletePrize({
  ...
  return withTransaction(async client=>{
    ...
    const selected=await client.query(
      "SELECT 1 FROM prize_selections WHERE prize_id=$1 LIMIT 1",
      [prizeId]
    );
    if(selected.rowCount || before.rows[0].status==="chosen"){
      await client.query(
        "UPDATE prizes SET status='disabled',updated_at=NOW() WHERE id=$1",
        [prizeId]
      );
    } else {
    ...
    await client.query(
      `INSERT INTO admin_audit_logs
       (id,admin_id,action,entity_type,entity_id,description,metadata)
       VALUES ($1,$2,'prize_removed','prize',$3,$4,$5::jsonb)`,
      [
        id("audit"),adminId,prizeId,
        selected.rowCount ? "Lot désactivé car déjà lié à un choix" : "Lot supprimé",
        JSON.stringify({competitionId,before:before.rows[0]})
      ]
    );
    return {ok:true,disabled:Boolean(selected.rowCount)};
  });
}
...
export async function movePrize({
  ...
  return withTransaction(async client=>{
    ...
    const idx=rows.rows.findIndex(r=>r.id===prizeId);
    if(idx<0) return {ok:false,reason:"not_found"};
    const target=dir==="up"?idx-1:idx+1;
    if(target<0 || target>=rows.rows.length) return {ok:true,noChange:true};

    for(let i=0;i<rows.rows.length;i++){
      await client.query(
        "UPDATE prizes SET sort_order=$2,updated_at=NOW() WHERE id=$1",
        [rows.rows[i].id,i]
      );
    }
    const currentId=rows.rows[idx].id;
    const targetId=rows.rows[target].id;
    await client.query("UPDATE prizes SET sort_order=$2,updated_at=NOW() WHERE id=$1",[currentId,target]);
    await client.query("UPDATE prizes SET sort_order=$2,updated_at=NOW() WHERE id=$1",[targetId,idx]);

    await client.query(
      `INSERT INTO admin_audit_logs
       (id,admin_id,action,entity_type,entity_id,description,metadata)
       VALUES ($1,$2,'prize_reordered','prize',$3,'Ordre des lots modifié',$4::jsonb)`,
      [id("audit"),adminId,prizeId,JSON.stringify({competitionId,direction:dir})]
    );
    return {ok:true};
  });
}
...
export async function updateRewardTier({
  ...
  return withTransaction(async client=>{
    const before=await client.query(
      "SELECT * FROM reward_tiers WHERE id=$1 AND competition_id=$2 FOR UPDATE",
      [tierId,competitionId]
    );
    if(!before.rowCount) return {ok:false,reason:"not_found"};
      ...
      const overlap=await client.query(
        `SELECT id,min_points,max_points
         ...
         WHERE competition_id=$1
           AND id<>$2
           AND enabled=TRUE
           AND min_points <= COALESCE($4::int,2147483647)
           AND COALESCE(max_points,2147483647) >= $3
         LIMIT 1`,
        [competitionId,tierId,min,max]
      );
      if(overlap.rowCount){
        return {ok:false,reason:"overlap",conflict:overlap.rows[0]};
      }
    ...
    await client.query(
      `UPDATE reward_tiers
       SET min_points=$3,max_points=$4,reward_type=$5,reward_value=$6,
           validity_days=$7,eligible_services=$8::jsonb,conditions=$9,
           sort_order=$10,enabled=$11,updated_at=NOW()
       WHERE id=$1 AND competition_id=$2`,
      [
        tierId,competitionId,min,max,String(rewardType||"discount").slice(0,40),
        Number(rewardValue)||0,
        String(validityDays??"").trim()===""?null:Math.max(1,Number.parseInt(String(validityDays),10)||1),
        JSON.stringify(Array.isArray(eligibleServices)?eligibleServices:[]),
        String(conditions||"").trim().slice(0,1000),
    ...
    await client.query(
      `INSERT INTO admin_audit_logs
       (id,admin_id,action,entity_type,entity_id,description,metadata)
       VALUES ($1,$2,'reward_tier_updated','reward_tier',$3,'Palier modifié',$4::jsonb)`,
      [id("audit"),adminId,tierId,JSON.stringify({competitionId,before:before.rows[0]})]
    );
    return {ok:true};
  });
}
...
export async function deleteRewardTier({
  ...
  return withTransaction(async client=>{
    ...
    const used=await client.query(
      "SELECT 1 FROM coupons WHERE reward_tier_id=$1 LIMIT 1",
      [tierId]
    );
    if(used.rowCount){
      await client.query(
        "UPDATE reward_tiers SET enabled=FALSE,updated_at=NOW() WHERE id=$1",
        [tierId]
      );
    } else {
      await client.query("DELETE FROM reward_tiers WHERE id=$1",[tierId]);
    }
    await client.query(
      `INSERT INTO admin_audit_logs
       (id,admin_id,action,entity_type,entity_id,description,metadata)
       VALUES ($1,$2,'reward_tier_removed','reward_tier',$3,$4,$5::jsonb)`,
      [
        id("audit"),adminId,tierId,
        used.rowCount ? "Palier désactivé car déjà utilisé par des coupons" : "Palier supprimé",
        JSON.stringify({competitionId,before:before.rows[0]})
      ]
    );
...
export async function moveRewardTier({
  competitionId,
  tierId,
  direction,
  adminId = "admin"
  ...
  return withTransaction(async client=>{
    const rows=await client.query(
      `SELECT id,sort_order,min_points,created_at
       FROM reward_tiers
       WHERE competition_id=$1
       ORDER BY sort_order ASC,min_points ASC,created_at ASC
       FOR UPDATE`,
      [competitionId]
    );
    const idx=rows.rows.findIndex(r=>r.id===tierId);
    if(idx<0) return {ok:false,reason:"not_found"};
    const target=dir==="up"?idx-1:idx+1;
    if(target<0 || target>=rows.rows.length) return {ok:true,noChange:true};
    ...
    for(let i=0;i<rows.rows.length;i++){
      await client.query(
        "UPDATE reward_tiers SET sort_order=$2,updated_at=NOW() WHERE id=$1",
        [rows.rows[i].id,i]
      );
    }
    const currentId=rows.rows[idx].id;
    const targetId=rows.rows[target].id;
    await client.query("UPDATE reward_tiers SET sort_order=$2,updated_at=NOW() WHERE id=$1",[currentId,target]);
    await client.query("UPDATE reward_tiers SET sort_order=$2,updated_at=NOW() WHERE id=$1",[targetId,idx]);

    await client.query(
      `INSERT INTO admin_audit_logs
       (id,admin_id,action,entity_type,entity_id,description,metadata)
       VALUES ($1,$2,'reward_tier_reordered','reward_tier',$3,'Ordre des paliers modifié',$4::jsonb)`,
      [id("audit"),adminId,tierId,JSON.stringify({competitionId,direction:dir})]
    );
    return {ok:true};
  });
}
