...
export async function claimDailyReward({
  ...
  const dayResult = await query(
    `SELECT d.id,d.day_number,d.title,d.reward_daily_points,d.status,d.starts_at,d.ends_at,
  ...
  await query(
    `INSERT INTO daily_claims
      (id, competition_id, day_id, participant_id, points_awarded, point_transaction_id,
       day_number_snapshot, day_title_snapshot)
    ...
    [
      ...
      dayId,
      participantId,
      Number(award.transactionPoints ?? award.awarded ?? 0),
      award.transactionId || null,
      Number(day.day_number||0) || null,
      String(day.title||"").slice(0,160) || null
    ]
  );
...
export async function submitMissionCompletion({
  ...
  const inserted=await query(
    `INSERT INTO mission_completions
      (id,mission_id,competition_id,participant_id,status,proof_data,
       mission_title_snapshot,mission_points_snapshot,mission_multiplier_snapshot)
    ...
    [
      ...
      String(m.title||"").slice(0,160),
      Number(m.points_fixed||0),
      Number(m.multiplier||1)
    ]
  );

  let effectiveCompletionId=completionId;
  let duplicate=!inserted.rowCount;
  let existingStatus="pending";

  if(!inserted.rowCount){
    const existingCompletion=await query(
      `SELECT id,status,point_transaction_id
       FROM mission_completions
       WHERE mission_id=$1 AND participant_id=$2
       LIMIT 1`,
      [missionId,participantId]
    );
    const existingRow=existingCompletion.rows[0];
    if(!existingRow){
      return {ok:false,reason:"completion_conflict"};
    }
    effectiveCompletionId=existingRow.id;
    existingStatus=existingRow.status;
    if(existingStatus!=="pending" || m.validation_mode!=="automatic"){
      return {
        ok:true,
        duplicate:true,
  ...
  if(m.validation_mode==="automatic"){
    award=await awardLedgerEvent({
      ...
      sourceId:effectiveCompletionId,
      idempotencyKey:"mission:"+effectiveCompletionId,
      description:"Mission automatique : "+m.title,
      createdBy:"system",
      metadata:{completionId:effectiveCompletionId,missionId}
    });
    ...
    if(award.inserted || award.reason==="duplicate"){
      await query(
        `UPDATE mission_completions
         ...
         WHERE id=$1
           AND status='pending'`,
        [effectiveCompletionId,award.transactionId||null]
      );
  ...
  return {
    ...
    completionId:effectiveCompletionId,
    validationMode:m.validation_mode,
    status:m.validation_mode==="automatic" && (award.inserted || award.reason==="duplicate") ? "confirmed" : existingStatus,
    awarded:Number(award.awarded||0),
...
export async function listMissionCompletions(competitionId) {
  const r = await query(
    `SELECT mc.id, mc.status, mc.proof_data, mc.created_at,
            m.id AS mission_id,
            COALESCE(m.title,mc.mission_title_snapshot,'Mission supprimée') AS mission_title,
            COALESCE(m.points_fixed,mc.mission_points_snapshot,0) AS points_fixed,
            COALESCE(m.multiplier,mc.mission_multiplier_snapshot,1) AS multiplier,
            p.id AS participant_id, p.pseudonym,
            cp.referral_code
     FROM mission_completions mc
     LEFT JOIN missions m ON m.id = mc.mission_id
...
export async function moveCompetitionDay({
  ...
  return withTransaction(async client=>{
    ...
    if(target.rowCount){
      ...
      await client.query(
        "UPDATE competition_days SET day_number=$2 WHERE id=$1",
        [src.id,temp]
      );
      await client.query(
        `UPDATE competition_days
         SET day_number=$2,
             starts_at=$3,
             ends_at=$4,
             updated_at=NOW()
         WHERE id=$1`,
        [targetRow.id,src.day_number,src.starts_at,src.ends_at]
      );
      await client.query(
        `UPDATE competition_days
         SET day_number=$2,
             ...
             updated_at=NOW()
         WHERE id=$1`,
        [src.id,targetNumber,targetRow.starts_at,targetRow.ends_at]
      );
    }else{
      const comp=await client.query(
        "SELECT starts_at FROM competitions WHERE id=$1 LIMIT 1",
        [competitionId]
      );
      let slotStart=src.starts_at;
      let slotEnd=src.ends_at;
      if(comp.rows[0]?.starts_at){
        const slot=await client.query(
          `SELECT
             $1::timestamptz + (($2::int - 1) * interval '1 day') AS starts_at,
             $1::timestamptz + ($2::int * interval '1 day') AS ends_at`,
          [comp.rows[0].starts_at,targetNumber]
        );
        slotStart=slot.rows[0].starts_at;
        slotEnd=slot.rows[0].ends_at;
      }
      await client.query(
        `UPDATE competition_days
         SET day_number=$2,starts_at=$3,ends_at=$4,updated_at=NOW()
         WHERE id=$1`,
        [src.id,targetNumber,slotStart,slotEnd]
      );
    }
