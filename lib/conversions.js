...
export async function createInterest({
  ...
  const campaignResult = await query(
    `SELECT id,product,name,points_interest,multiplier,status,whatsapp_url,destination_url
     FROM campaigns
     WHERE id=$1
  ...
  const attribution=await query(
    `SELECT va.participant_id,va.referral_code
     FROM visitor_attributions va
     JOIN competition_participants cp
       ON cp.competition_id=va.competition_id
      AND cp.participant_id=va.participant_id
     JOIN participants p ON p.id=va.participant_id
     WHERE va.competition_id=$1
       AND va.visitor_id_hash=$2
  ...
  let participantId=attribution.rows[0]?.participant_id || null;
  let attributedReferralCode=attribution.rows[0]?.referral_code || null;

  if(!participantId){
    const fallback=await query(
      `SELECT cp.participant_id,cp.referral_code
    ...
    participantId=fallback.rows[0]?.participant_id || null;
    attributedReferralCode=fallback.rows[0]?.referral_code || null;
  }
  ...
  const existing = await query(
    `SELECT id,reference_code,status,source_participant_id
     FROM interests
     WHERE campaign_id=$1
  ...
  if(existing.rowCount){
    return {
      ...
      participantId:existing.rows[0].source_participant_id || participantId,
  ...
  const reference=referenceCode(campaign.product,attributedReferralCode);

  const insertedInterest=await query(
    `INSERT INTO interests
     ...
     ON CONFLICT (campaign_id,visitor_id_hash) DO NOTHING
     RETURNING id`,
    [
      interestId,
      competitionId,
      ...
      reference,
      JSON.stringify({
        clickedReferralCode:referralCode,
        attributedReferralCode,
        attributionModel:"competition_first_touch"
      })
    ]
  );

  if(!insertedInterest.rowCount){
    const raced=await query(
      `SELECT id,reference_code,status,source_participant_id
       FROM interests
       WHERE campaign_id=$1 AND visitor_id_hash=$2
       LIMIT 1`,
      [campaignId,visitorHash]
    );
    ...
    if(existingRow){
      return {
        ok:true,
        duplicate:true,
        interestId:existingRow.id,
        referenceCode:existingRow.reference_code,
        status:existingRow.status,
        participantId:existingRow.source_participant_id || participantId,
        campaignName:campaign.name,
        product:campaign.product,
        whatsappUrl:campaign.whatsapp_url,
        destinationUrl:campaign.destination_url
  ...
  if(Number(campaign.points_interest||0)!==0){
    award=await awardLedgerEvent({
      ...
      eventId:interestId,
      type:"interest",
      basePoints:Number(campaign.points_interest||0),
