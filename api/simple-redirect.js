...
async function ensureFivePointClickRule(competitionId){await query(`INSERT INTO point_rules (id,competition_id,action_type,enabled,base_points,multiplier,daily_cap_points,settings) VALUES ('rule_simple_'||substr(md5($1),1,20),$1,'valid_click',TRUE,$2,1,NULL,'{"simpleMode":true}'::jsonb) ON CONFLICT (competition_id,action_type) DO UPDATE SET enabled=TRUE,base_points=EXCLUDED.base_points,multiplier=1,daily_cap_points=NULL,settings=COALESCE(point_rules.settings,'{}'::jsonb)||'{"simpleMode":true}'::jsonb,updated_at=NOW()`,[competitionId,CLICK_POINTS]);}
export default async function handler(req,res){
 ...
 try{
  ...
  if(!adminSession&&tracked?.raw){await query(`UPDATE competition_participants SET raw_clicks_cache=raw_clicks_cache+1,unique_clicks_cache=unique_clicks_cache+$3,valid_clicks_cache=valid_clicks_cache+$4 WHERE competition_id=$1 AND referral_code=$2`,[competitionId,referralCode,tracked.isUnique?1:0,tracked.isValid?1:0]).catch(()=>{});}
