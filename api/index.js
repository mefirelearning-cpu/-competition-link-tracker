...
import { listCompetitionDays, getActiveDay, listDayMissions, createCompetitionDay, createMission, claimDailyReward, addStreakRule, setStreakRuleEnabled, deleteStreakRule, submitMissionCompletion, listMissionCompletions, reviewMissionCompletion, duplicateCompetitionDay, deleteCompetitionDay, moveCompetitionDay } from "../lib/competition-days.js";
...
async function participantDashboardPage(origin, session) {
  ...
  const missionHtml = day
    ? "<div class=\"p-card p-span6\"><div class=\"p-kicker\" style=\"color:#666\">Événement du jour</div><h2 style=\"font-size:25px;margin:7px 0 8px\">" + esc(day.title) + "</h2><p class=\"p-muted\" style=\"margin:0 0 12px\">" + esc(day.description||day.marketing_message||"Consulte la mission et partage la campagne vedette.") + "</p>" +
      (day.notification_text ? "<div class=\"p-notice\" style=\"margin-bottom:10px\"><b>Annonce du jour</b><br>" + esc(day.notification_text) + "</div>" : "") +
...
function leaderboardRowsHtml(rows, origin, comp, publicMode, viewerCode = "") {
  if (!rows.length) return "<tr><td colspan=\"" + (publicMode ? "5" : "8") + "\" class=\"empty\">Aucun participant.</td></tr>";
  return rows.map((r) => {
    const link = origin + r.link;
    if (publicMode) {
      const winnerCount=Math.max(1,Number(comp.winnerCount||1));
      const rowClass=[
        r.rank<=3?"top-three":"",
        r.rank<=winnerCount?"winner-zone":"",
        viewerCode && r.code===viewerCode?"viewer-row":""
      ].filter(Boolean).join(" ");
      return "<tr class=\"" + rowClass + "\" data-code=\"" + esc(r.code) + "\">" +
        "<td class=\"rank " + (r.rank === 1 ? "one" : "") + "\">" + r.rank + "</td>" +
        "<td><div class=\"person\">" + esc(r.name) + (viewerCode && r.code===viewerCode?" <span class=\"viewer-badge\">TOI</span>":"") + "</div><div class=\"code\">" + esc(r.code) + "</div></td>" +
...
async function adminDaysContent(comp) {
  const id = encodeURIComponent(comp.id);
  const [days,campaigns,config,completions,streakResult] = await Promise.all([
    listCompetitionDays(comp.id),
    listCampaigns(comp.id,{admin:true}),
    getCompetitionConfig(comp.id),
    listMissionCompletions(comp.id),
    query("SELECT * FROM streak_rules WHERE competition_id=$1 ORDER BY consecutive_days ASC",[comp.id])
  ]);
  const streaks=streakResult.rows;
  const campaignOptions = "<option value=\"\">Aucune</option>" + campaigns.map(x=>"<option value=\"" + esc(x.id) + "\">" + esc(x.name) + "</option>").join("");
  const formatDate=v=>v?new Date(v).toISOString().slice(0,16):"";
  ...
  const dayCards = days.length ? days.map(day =>
    "<article class=\"card span6\"><div class=\"section-title\"><div><h2>Jour " + day.day_number + " · " + esc(day.title) + "</h2><div class=\"subnav-note\">" + esc(day.status) + (day.featured_campaign_name ? " · " + esc(day.featured_campaign_name) : "") + "</div></div><span class=\"score-badge\">+" + Number(day.reward_daily_points||0) + " pts</span></div>" +
      ...
      ((day.starts_at||day.ends_at) ? "<div class=\"notice\" style=\"margin-bottom:12px\"><b>Programmation</b><br>" + esc(day.starts_at?new Date(day.starts_at).toLocaleString("fr-FR"):"—") + " → " + esc(day.ends_at?new Date(day.ends_at).toLocaleString("fr-FR"):"—") + "</div>" : "") +
      "<details><summary class=\"btn2\" style=\"cursor:pointer;display:inline-flex\">Ajouter une mission</summary><form method=\"post\" action=\"/api/competition/" + id + "/mission-create\" style=\"margin-top:10px\"><input type=\"hidden\" name=\"dayId\" value=\"" + esc(day.id) + "\"><div class=\"form-grid\"><div><label>Mission</label><input name=\"title\" placeholder=\"STATUS TAKEOVER\" required></div><div><label>Points</label><input name=\"pointsFixed\" type=\"number\" value=\"0\"></div><div><label>Multiplicateur</label><input name=\"multiplier\" type=\"number\" min=\"0\" step=\"0.1\" value=\"1\"></div><div><label>Validation</label><select name=\"validationMode\"><option value=\"manual\">Manuelle</option><option value=\"automatic\">Automatique</option></select></div><div><label>Maximum de complétions</label><input name=\"maxCompletions\" type=\"number\" min=\"1\" placeholder=\"Vide = illimité\"></div><div><label>Statut</label><select name=\"status\"><option value=\"draft\">Brouillon</option><option value=\"scheduled\">Programmée</option><option value=\"active\">Active</option></select></div><div><label>Campagne associée</label><select name=\"campaignId\">" + campaignOptions + "</select></div><div><label>Début</label><input class=\"config-datetime\" name=\"startsAt\" type=\"datetime-local\"></div><div><label>Fin</label><input class=\"config-datetime\" name=\"endsAt\" type=\"datetime-local\"></div><div class=\"full\"><label>Description</label><textarea name=\"description\"></textarea></div><div class=\"full\"><button class=\"btn2\" type=\"submit\">Enregistrer la mission</button></div></div></form></details>" +
      "<div class=\"actions\" style=\"margin-top:12px\"><form method=\"post\" action=\"/api/competition/" + id + "/day-move\"><input type=\"hidden\" name=\"dayId\" value=\"" + esc(day.id) + "\"><button class=\"btn2\" name=\"direction\" value=\"up\" type=\"submit\">Monter</button><button class=\"btn2\" name=\"direction\" value=\"down\" type=\"submit\">Descendre</button></form><form method=\"post\" action=\"/api/competition/" + id + "/day-duplicate\"><input type=\"hidden\" name=\"dayId\" value=\"" + esc(day.id) + "\"><button class=\"btn2\" type=\"submit\">Dupliquer</button></form><form method=\"post\" action=\"/api/competition/" + id + "/day-delete\"><input type=\"hidden\" name=\"dayId\" value=\"" + esc(day.id) + "\"><button class=\"danger\" type=\"submit\">Supprimer</button></form></div>" +
    "</article>"
...
async function adminRewardsContent(comp) {
  ...
  const prizeRows = prizes.length ? prizes.map(p=>
    "<tr><td><div class=\"person\">" + esc(p.name) + "</div><div class=\"code\">ordre " + Number(p.sort_order||0) + "</div></td>" +
    "<td>" + esc(p.duration_text||"") + "</td><td>" + esc(p.status) + "</td><td>" + esc(p.chosen_by_name||"—") + "</td>" +
    "<td><div class=\"actions\">" +
      "<form method=\"post\" action=\"/api/competition/" + id + "/prize-move\"><input type=\"hidden\" name=\"prizeId\" value=\"" + esc(p.id) + "\"><button class=\"btn2\" name=\"direction\" value=\"up\" type=\"submit\">↑</button><button class=\"btn2\" name=\"direction\" value=\"down\" type=\"submit\">↓</button></form>" +
      "<details><summary class=\"btn2\" style=\"cursor:pointer\">Modifier</summary><form method=\"post\" action=\"/api/competition/" + id + "/prize-update\" style=\"margin-top:8px;min-width:260px\"><input type=\"hidden\" name=\"prizeId\" value=\"" + esc(p.id) + "\"><div class=\"form-grid\"><div class=\"full\"><label>Lot</label><input name=\"name\" value=\"" + esc(p.name) + "\" required></div><div><label>Durée</label><input name=\"durationText\" value=\"" + esc(p.duration_text||"") + "\"></div><div><label>Ordre</label><input name=\"sortOrder\" type=\"number\" value=\"" + Number(p.sort_order||0) + "\"></div><div class=\"full\"><label>Description</label><textarea name=\"description\">" + esc(p.description||"") + "</textarea></div><div class=\"full\"><button class=\"btn\" type=\"submit\">Enregistrer</button></div></div></form></details>" +
      "<form method=\"post\" action=\"/api/competition/" + id + "/prize-delete\"><input type=\"hidden\" name=\"prizeId\" value=\"" + esc(p.id) + "\"><button class=\"danger\" type=\"submit\">" + (p.status==="chosen"?"Désactiver":"Supprimer") + "</button></form>" +
    "</div></td></tr>"
  ).join("") : "<tr><td colspan=\"5\" class=\"empty\">Aucun lot.</td></tr>";

  const tierRows = tiers.length ? tiers.map(t=>{
    const services=Array.isArray(t.eligible_services)?t.eligible_services.join(", "):"";
    return "<tr><td>" + Number(t.min_points) + "</td><td>" + (t.max_points===null?"∞":Number(t.max_points)) + "</td><td>" + esc(t.reward_type) + "</td><td>" + Number(t.reward_value) + "</td><td>" + (t.validity_days||"—") + "</td><td>" + esc(services||"Tous") + "</td>" +
      "<td><div class=\"actions\">" +
        "<form method=\"post\" action=\"/api/competition/" + id + "/tier-move\"><input type=\"hidden\" name=\"tierId\" value=\"" + esc(t.id) + "\"><button class=\"btn2\" name=\"direction\" value=\"up\" type=\"submit\">↑</button><button class=\"btn2\" name=\"direction\" value=\"down\" type=\"submit\">↓</button></form>" +
        "<details><summary class=\"btn2\" style=\"cursor:pointer\">Modifier</summary><form method=\"post\" action=\"/api/competition/" + id + "/tier-update\" style=\"margin-top:8px;min-width:300px\"><input type=\"hidden\" name=\"tierId\" value=\"" + esc(t.id) + "\"><div class=\"form-grid\"><div><label>Minimum</label><input name=\"minPoints\" type=\"number\" min=\"0\" value=\"" + Number(t.min_points||0) + "\" required></div><div><label>Maximum</label><input name=\"maxPoints\" type=\"number\" min=\"0\" value=\"" + (t.max_points??"") + "\"></div><div><label>Type</label><select name=\"rewardType\"><option value=\"discount\"" + (t.reward_type==="discount"?" selected":"") + ">Réduction %</option><option value=\"credit\"" + (t.reward_type==="credit"?" selected":"") + ">Crédit</option><option value=\"custom\"" + (t.reward_type==="custom"?" selected":"") + ">Personnalisé</option></select></div><div><label>Valeur</label><input name=\"rewardValue\" type=\"number\" step=\"0.01\" value=\"" + Number(t.reward_value||0) + "\" required></div><div><label>Validité</label><input name=\"validityDays\" type=\"number\" min=\"1\" value=\"" + (t.validity_days??"") + "\"></div><div><label>Ordre</label><input name=\"sortOrder\" type=\"number\" value=\"" + Number(t.sort_order||0) + "\"></div><div class=\"full\"><label>Services</label><input name=\"eligibleServices\" value=\"" + esc(services) + "\"></div><div class=\"full\"><label>Conditions</label><textarea name=\"conditions\">" + esc(t.conditions||"") + "</textarea></div><div class=\"full\"><label><input type=\"checkbox\" name=\"enabled\" value=\"1\" style=\"width:auto;margin-right:7px\"" + (t.enabled?" checked":"") + "> Actif</label></div><div class=\"full\"><button class=\"btn\" type=\"submit\">Enregistrer</button></div></div></form></details>" +
        "<form method=\"post\" action=\"/api/competition/" + id + "/tier-delete\"><input type=\"hidden\" name=\"tierId\" value=\"" + esc(t.id) + "\"><button class=\"danger\" type=\"submit\">Supprimer</button></form>" +
      "</div></td></tr>";
  }).join("") : "<tr><td colspan=\"7\" class=\"empty\">Aucun palier.</td></tr>";
  ...
  return pageTitleHtml("Récompenses","Lots, ordre de choix, paliers modifiables et coupons à usage unique.") +
    "<div class=\"grid\">" +
      ...
      "<div class=\"card span12\"><div class=\"section-title\"><h2>Lots</h2></div><div class=\"table-wrap\"><table><thead><tr><th>Lot</th><th>Durée</th><th>Statut</th><th>Choisi par</th><th>Actions</th></tr></thead><tbody>" + prizeRows + "</tbody></table></div></div>" +
      "<div class=\"card span12\"><div class=\"section-title\"><h2>Paliers</h2></div><div class=\"table-wrap\"><table><thead><tr><th>Min</th><th>Max</th><th>Type</th><th>Valeur</th><th>Jours</th><th>Services</th><th>Actions</th></tr></thead><tbody>" + tierRows + "</tbody></table></div></div>" +
      "<div class=\"card span12\"><div class=\"section-title\"><div><h2>Sélection des gagnants</h2><div class=\"subnav-note\">Le gagnant #1 choisit avant le #2, puis ainsi de suite. L’administrateur peut attribuer un lot en cas d’absence de réponse.</div></div></div><div class=\"table-wrap\"><table><thead><tr><th>Rang</th><th>Participant</th><th>Points</th><th>Choix admin</th></tr></thead><tbody>" + winnerRows + "</tbody></table></div></div>" +
...
async function competitionPage(origin, comp, view = "overview", publicMode = false, newCode = "", viewerCode = "") {
  const rows = await getRankedParticipants(comp);
  ...
  if (publicMode) {
    ...
    content = publicHeader + publicKpis +
      "<div class=\"public-board\"><div class=\"public-panel\"><div class=\"section-title\"><div><h2>Classement général</h2><div class=\"subnav-note\">Du premier au dernier · zone gagnante Top " + Math.max(1,Number(comp.winnerCount||1)) + " · priorité aux points</div></div><div class=\"live\"><span class=\"pulse\"></span><span id=\"updatedAt\">live</span></div></div>" +
        "<div class=\"table-wrap\"><table class=\"public-table\"><thead><tr><th>#</th><th>Participant</th><th>Points</th><th>Valides</th><th>Personnes</th></tr></thead><tbody id=\"leaderboardBody\">" + leaderboardRowsHtml(rows, origin, comp, true, viewerCode) + "</tbody></table></div>" +
  ...
  const script =
    "const endpoint=" + JSON.stringify(endpoint) + ";const origin=" + JSON.stringify(origin) + ";const publicMode=" + JSON.stringify(publicMode) + ";const initialViewerCode=" + JSON.stringify(viewerCode) + ";" +
    ...
    "function render(data){const rows=data.participants||[];const a=document.getElementById('statParticipants'),p=document.getElementById('statPoints'),b=document.getElementById('statClicks'),u=document.getElementById('statUnique'),v=document.getElementById('statValid'),l=document.getElementById('statLeader');if(a)a.textContent=rows.length;if(p)p.textContent=data.totals.points||0;if(b)b.textContent=data.totals.clicks;if(u)u.textContent=data.totals.unique;if(v)v.textContent=data.totals.valid||0;if(l)l.textContent=rows[0]?rows[0].name:'—';" +
      "const body=document.getElementById('leaderboardBody');if(body){body.textContent='';const viewerCode=data.viewerCode||initialViewerCode||'';const winnerCount=Number(data.competition&&data.competition.winnerCount||1);rows.forEach(r=>{const tr=document.createElement('tr');if(publicMode){tr.className=[r.rank<=3?'top-three':'',r.rank<=winnerCount?'winner-zone':'',viewerCode&&r.code===viewerCode?'viewer-row':''].filter(Boolean).join(' ')}tr.appendChild(el('td','rank'+(r.rank===1?' one':''),String(r.rank)));const tdP=el('td');const person=el('div','person',r.name);if(publicMode&&viewerCode&&r.code===viewerCode){person.appendChild(el('span','viewer-badge','TOI'))}tdP.append(person,el('div','code',r.code));tr.appendChild(tdP);tr.appendChild(el('td','points-col',String(r.points)+' pts'));tr.appendChild(el('td','',String(publicMode?r.valid:r.clicks)));const tdU=el('td');tdU.appendChild(el('b','',String(r.unique)));tr.appendChild(tdU);if(!publicMode){tr.appendChild(el('td','',String(r.valid)));const tdL=el('td');const ac=el('div','actions');const cp=el('button','iconbtn copy','Copier');cp.type='button';cp.dataset.link=origin+r.link;const op=el('a','iconbtn','Ouvrir');op.href=origin+r.link;op.target='_blank';ac.append(cp,op);tdL.appendChild(ac);tr.appendChild(tdL);const td=el('td');const fm=document.createElement('form');fm.method='post';fm.action='/api/competition/'+encodeURIComponent(data.competition.id)+'/delete-participant';const input=document.createElement('input');input.type='hidden';input.name='code';input.value=r.code;const bt=el('button','danger','Supprimer');bt.type='submit';fm.append(input,bt);td.appendChild(fm);tr.appendChild(td)}body.appendChild(tr)})}" +
    ...
    "document.querySelectorAll('.campaign-image-file').forEach(cf=>{cf.addEventListener('change',()=>{const file=cf.files&&cf.files[0];if(!file)return;if(file.size>12000000){alert('Image trop lourde. Choisis une image de moins de 12 Mo.');cf.value='';return}const reader=new FileReader();reader.onload=()=>{const img=new Image();img.onload=()=>{const max=1400;const ratio=Math.min(1,max/Math.max(img.width,img.height));const canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(img.width*ratio));canvas.height=Math.max(1,Math.round(img.height*ratio));const ctx=canvas.getContext('2d');ctx.drawImage(img,0,0,canvas.width,canvas.height);const data=canvas.toDataURL('image/jpeg',0.82);const target=document.getElementById(cf.dataset.target||'campaignImageData');if(target)target.value=data};img.src=reader.result};reader.readAsDataURL(file)})});" +
    "document.querySelectorAll('.config-datetime').forEach(input=>{input.form&&input.form.addEventListener('submit',()=>{if(input.value&&input.name){try{const h=document.createElement('input');h.type='hidden';h.name=input.name;h.value=new Date(input.value).toISOString();input.removeAttribute('name');input.form.appendChild(h)}catch{}}})});" ;
...
async function apiStats(req, res, comp) {
  ...
  const viewerSession = await getParticipantSession(req, comp.id).catch(()=>null);
  return send(res, 200, JSON.stringify({
    competition: {
      ...
      winnerCount: Number(config?.winner_count || comp.winnerCount || 1)
    },
    viewerCode: viewerSession?.referral_code || "",
    totals: {
...
export default async function handler(req, res) {
  try {
    ...
    if (path.startsWith("leaderboard/")) {
      ...
      const viewerSession=await getParticipantSession(req,id).catch(()=>null);
      return send(res, 200, await competitionPage(origin, comp, "ranking", true, "", viewerSession?.referral_code||""));
    }
    ...
    if (match) {
      ...
      if (action === "mission-create" && req.method === "POST") {
        ...
        await createMission({
          ...
          startsAt: b.startsAt || null,
          endsAt: b.endsAt || null,
          maxCompletions: b.maxCompletions,
          adminId: "admin"
        });
        return redirect(res, "/c/" + encodeURIComponent(id) + "/days", 303);
      }
      ...
      if (action === "status" && req.method === "POST") {
        ...
        const allowed = ["active","scheduled","paused","ended","archived","draft"];
