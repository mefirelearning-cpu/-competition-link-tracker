...
async function competitionPage(origin, comp, view = "overview", publicMode = false, newCode = "") {
  ...
  const script =
    ...
    "document.querySelectorAll('.config-datetime').forEach(input=>{input.form&&input.form.addEventListener('submit',()=>{if(input.value){try{input.value=new Date(input.value).toISOString()}catch{}}})});" ;
