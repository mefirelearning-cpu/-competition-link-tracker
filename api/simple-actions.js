...
export default async function handler(req,res){
 ...
 if(raw==="api/simple/competition/create"){
  try{const name=clean(req.body?.name).slice(0,100);if(!name)return go(res,"/admin?error="+encodeURIComponent("Nom de compétition requis"));const competitionId=id("cmp");let slug=slugify(name)||"competition";slug+="-"+randomBytes(3).toString("hex");await query(`INSERT INTO competitions (id,slug,name,status,registrations_open,leaderboard_visible) VALUES ($1,$2,$3,'active',true,true)`,[competitionId,slug,name]);return go(res,"/c/"+encodeURIComponent(competitionId)+"?createdCompetition=1");}catch(e){console.error("create competition:",e);return go(res,"/admin?error="+encodeURIComponent("Création impossible"));}
 }
 const m=raw.match(/^api\/simple\/competition\/([^/]+)\/(add|bulk|points)$/);if(!m){res.statusCode=404;return res.end("Not found");}
 const competitionId=decodeURIComponent(m[1]),action=m[2],back="/c/"+encodeURIComponent(competitionId);
 try{
  if(action==="add"){
   const name=clean(req.body?.name);let referral=clean(req.body?.code).toLowerCase().replace(/[^a-z0-9_-]/g,"").slice(0,32);if(!name)return go(res,back+"?error="+encodeURIComponent("Nom requis"));if(!referral)referral=randomCode();await query("BEGIN");try{await addParticipant(competitionId,name,referral);await query("COMMIT");}catch(e){await query("ROLLBACK");throw e;}return go(res,back+"?created="+encodeURIComponent(referral));
  }
  if(action==="bulk"){
   const count=Math.max(1,Math.min(200,Math.trunc(Number(req.body?.count)||0)));if(!count)return go(res,back+"?error="+encodeURIComponent("Nombre invalide"));const existing=await query(`SELECT p.pseudonym FROM competition_participants cp JOIN participants p ON p.id=cp.participant_id WHERE cp.competition_id=$1 AND p.pseudonym ~ '^P-[0-9]+$'`,[competitionId]);let max=0;for(const r of existing.rows){const n=Number(String(r.pseudonym).split("-")[1]);if(Number.isFinite(n))max=Math.max(max,n);}await query("BEGIN");try{for(let i=1;i<=count;i++){const n=max+i,name=`P-${String(n).padStart(3,"0")}`;await addParticipant(competitionId,name,randomCode());}await query("COMMIT");}catch(e){await query("ROLLBACK");throw e;}return go(res,back+"?bulk="+count);
  }
